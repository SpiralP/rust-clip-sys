...
static void print_channel(const clip::image& img,
  ...
  for (unsigned long y = 0; y < spec.height; ++y) {
    ...
    for (unsigned long x = 0; x < spec.width; ++x, p += pixel_size) {
      std::cout << std::right << std::hex << std::setw(channel_fill_width)
...
static void print_channels(const clip::image& img,
  ...
  print_channel<T>(img, spec, "Red", spec.red_mask, spec.red_shift,
                   channel_fill_width, pixel_size);
  print_channel<T>(img, spec, "Green", spec.green_mask, spec.green_shift,
                   channel_fill_width, pixel_size);
  print_channel<T>(img, spec, "Blue", spec.blue_mask, spec.blue_shift,
                   channel_fill_width, pixel_size);
  if (spec.alpha_mask)
    print_channel<T>(img, spec, "Alpha", spec.alpha_mask, spec.alpha_shift,
                     channel_fill_width, pixel_size);
}
...
int mainn() {
  ...
  std::cout << "Image in clipboard " << spec.width << "x" << spec.height << " ("
            << spec.bits_per_pixel << "bpp)\n"
            << "Format:"
            << "\n"
            << std::hex << "  Red   mask: " << spec.red_mask << "\n"
            << "  Green mask: " << spec.green_mask << "\n"
            << "  Blue  mask: " << spec.blue_mask << "\n"
            << "  Alpha mask: " << spec.alpha_mask << "\n"
            << std::dec << "  Red   shift: " << spec.red_shift << "\n"
  ...
  for (unsigned long y = 0; y < spec.height; ++y) {
    ...
    for (unsigned long x = 0; x < spec.width; ++x) {
      for (unsigned long byte = 0; byte < spec.bits_per_pixel / 8; ++byte, ++p)
        std::cout << std::right << std::hex << std::setfill('0') << std::setw(2)
                  << int((*p) & 0xff) << " ";
    }
    std::cout << "\n";
  }
